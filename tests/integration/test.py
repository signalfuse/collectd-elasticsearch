...
def wait_for_metrics_from_each_cluster():
    ...
    for cluster in VERSIONS_TESTED_WITH_METRICS:
        ...
        eventually_true(
            lambda: any([c in m.get("plugin_instance") for m in get_metric_data()]), TIMEOUT_SECS - (time() - start)
        )
        ...
        for metric in VERSIONS_TESTED_WITH_METRICS.get(cluster):
            ...
            eventually_true(
                lambda: any([metric in str(m.get("type_instance")) for m in get_metric_data()]),
